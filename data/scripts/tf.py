...
def create_model(data):
    ...
    l4 = tf.add(
        tf.matmul(l3, hidden_4_layer['weights']), hidden_4_layer['biases'])
    l4 = tf.nn.relu(l4)
...
def train_model(x):
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(epochs):
            # Get the epoch loss.
            _, epoch_loss = sess.run([optimizer, cost], feed_dict={
                                     x: train_files, y: train_labels})
        ...
        for item in test_files:
            # Set [1, 0] for the magic phrase
            if (accuracy.eval({x: [item[0]], y: [[1, 0]]})):
                print('Found magic phrase!' + '(' + item[1] + ')')
            else:
                print('Oops!')

        # Print all predictions.
        print('Predictions: ', prediction.eval(
            feed_dict={x: [item[0] for item in test_files]}, session=sess))
